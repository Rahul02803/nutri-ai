...
export default function DashboardScreen() {
  ...
  const handleLogMealSubmit = () => {
    ...
    const pro = parseFloat(mealProtein || "0");
    const carb = parseFloat(mealCarbs || "0");
    const fat = parseFloat(mealFat || "0");

    if (!mealName.trim()) {
      Alert.alert("Invalid Input", "Please enter a food name or description.");
      return;
    }
    if (isNaN(cal) || cal < 0) {
    ...
    Alert.alert("Meal Logged", `${mealName} has been logged successfully!`);
  };
              ...
              <View key={meal.id} style={styles.foodRow}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.foodRowName}>
                    {meal.foods && meal.foods[0] ? meal.foods[0].food_name : meal.meal_type}
                  </Text>
                  <Text style={styles.foodRowMacros}>
                    {meal.meal_type} • P: {Math.round(meal.protein)}g  C: {Math.round(meal.carbs)}g  F: {Math.round(meal.fat)}g
                  </Text>
                </View>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.mealTypeRow}>
              {(["Breakfast", "Lunch", "Dinner", "Snack"] as const).map((type) => (
                <TouchableOpacity
                  key={type}
                  style={[styles.mealTypeBtn, mealType === type && styles.mealTypeBtnSelected]}
                  onPress={() => setMealType(type)}
                >
                  <Text style={[styles.mealTypeBtnText, mealType === type && styles.mealTypeBtnTextSelected]}>
                    {type}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <Text style={styles.modalLabel}>FOOD OR MEAL NAME</Text>
            <TextInput
              style={styles.modalInput}
              placeholder="e.g. Paneer Tikka or Oats"
              placeholderTextColor="#6B7280"
              value={mealName}
              onChangeText={setMealName}
            />

            <Text style={[styles.modalLabel, { marginTop: 12 }]}>CALORIES (KCAL)</Text>
            <TextInput
              style={styles.modalInput}
            ...
            <View style={styles.modalButtons}>
              ...
              >
                <Text style={styles.modalBtnTextSubmit}>Log Meal</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
